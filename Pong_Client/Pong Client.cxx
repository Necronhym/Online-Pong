...
class CAudio
	{
		ALuint alintBuffer, alintSource;
		ALint alboolPlaying;
		public:
			//Load audio and play it:
			void Play( const char* charLocation, float floVolume, bool boolLoop = 0 )
				{
					alutInit( 0, NULL );
					alintBuffer = alutCreateBufferFromFile( charLocation );
					alGenSources( 1, &alintSource );
					alSourcei( alintSource, AL_BUFFER, alintBuffer );
					alSourcef( alintSource, AL_GAIN, floVolume );
					alSourcei( alintSource, AL_LOOPING, boolLoop );
					alSourcePlay( alintSource );
					alGetSourcei( alintSource, AL_SOURCE_STATE, &alboolPlaying );
					if( alboolPlaying != AL_PLAYING )
						{
							alDeleteSources( 1, &alintSource );
							alDeleteBuffers( 1, &alintBuffer );
						}
				}
	};
...
//Handles audio play on inpact wit hpaddle:
class CBall
	{
		CAudio Sound;
		public:
			void BallLogic( float floPlayer1Y, float floPlayer2Y, float floBallX, float floBallY )
				{
					if( floBallX <= -0.9 && floBallX >= -0.95 && floBallY <= floPlayer1Y+0.2 && floBallY >= floPlayer1Y-0.2 )
						{
							Sound.Play( "beep.wav", 0.5, 0 );
						}
		...
		while( !glfwGetKey( GLFW_KEY_ESC ) )
			{
				...
				Network.Recive();
				Ball.BallLogic( Mouse.floMouseY, Network.floPlayer2Y, Network.floBallX, Network.floBallY );
